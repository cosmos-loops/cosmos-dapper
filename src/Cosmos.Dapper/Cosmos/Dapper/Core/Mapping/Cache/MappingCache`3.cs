...
{
    internal static class MappingCache<T1, T2, T3>
    {
        static MappingCache()
        {
            ...
            Map = Expression.Lambda<Func<T1, T2, T3, T1>>(blockExpression, first, second, third).Compile();
        }

        internal static Func<T1, T2, T3, T1> Map { get; }
    }
}